...
def consume_oauth_state(state: str):
    ...
    Returns:
        The stored session dict (contains 'timestamp' and optionally 'redirect_uri')
        if the state was valid, or False if invalid/expired.
    """
    if state not in _oauth_sessions:
...
COGNITO_DOMAIN = os.getenv("COGNITO_DOMAIN")  # e.g., https://yourdomain.auth.us-east-1.amazoncognito.com
APP_REDIRECT_URI = os.getenv("APP_REDIRECT_URI")  # e.g., http://localhost:8080/auth/callback
...
class CognitoUser:
    ...
    def __init__(self, sub: str, email: str, name: str, groups: Optional[list] = None):
        self.sub = sub  # Unique user ID
        self.email = email
        self.name = name
        self.groups = groups or []
        self.user_room_id: Optional[str] = None  # Set after room is created/fetched
...
def get_cognito_jwks():
    ...
    try:
        ...
        raise ValueError("Failed to verify token. Try again later.")
...
def verify_cognito_token(token: str) -> CognitoUser:
    ...
    try:
        ...
        # Skip at_hash verification since we only have the ID token, not access token
        claims = jwt.decode(
            token,
            public_key,
            algorithms=["RS256"],
            audience=COGNITO_CLIENT_ID,
            options={"verify_at_hash": False},  # Skip at_hash check for ID tokens
        )
...
def get_cognito_logout_url(logout_redirect_uri: str = "http://localhost:8080/") -> str:
    """
    Generate the Cognito logout endpoint URL that terminates the Cognito session.
    Uses the OIDC standard parameter post_logout_redirect_uri.
    
    After logging out from Cognito, the user will be unable to auto-login with cached credentials.
    On next login, they will see the account selector.
    
    Args:
        logout_redirect_uri: Where to redirect after logout (default: home page)
    ...
    # Use post_logout_redirect_uri (OIDC standard) not logout_uri
    params = {
        "client_id": COGNITO_CLIENT_ID,
...
def exchange_code_for_token(code: str, redirect_uri: str = None) -> Optional[Dict[str, Any]]:
    """
    Exchange Cognito authorization code for tokens (ID token, access token, refresh token).
    
    Args:
        code: Authorization code from Cognito redirect
        redirect_uri: Override redirect URI (must exactly match what was sent to /oauth2/authorize)
    ...
    try:
        ...
        # Add client secret (we're a confidential backend client)
...
def get_user_attributes(username: str) -> Optional[Dict[str, str]]:
    """
    ...
    try:
        ...
        logger.error("Failed to get user attributes: %s", e)
        return None
...
def validate_cognito_config() -> bool:
    ...
    if missing:
        logger.error("Missing required Cognito config: %s", ", ".join(missing))
        return False
